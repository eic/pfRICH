...
// root [1] .x hepmc_writer_tof_pion.C+("out.hepmc", 1000)
//
...
void hepmc_writer_tof_pion(const char* out_fname, int n_events)
{
  ...
  for (events_parsed = 0; events_parsed < n_events; events_parsed++) {
    ...
    for(unsigned iq=0; iq</*2*/1; iq++) {
      Double_t eta   = -2.5;//rdmn_gen->Uniform(-2.5, -2.30);
      Double_t th    = 2*std::atan(exp(-eta));
      Double_t p     = rdmn_gen->Uniform(7.0, 7.1);
      Double_t phi   = 90.0*M_PI/180.;//rdmn_gen->Uniform(90.0*M_PI/180., 100.*M_PI/180.);
      ...
      auto particle = pion;//iq%2 ? pion : kaon;
