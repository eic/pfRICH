...
void hepmc_writer_two_particles_full(float pp1, float pp2, float et1, float et2, float pdg1, float pdg2, int n_events)
{
  auto *DatabasePDG = new TDatabasePDG();
  auto *pion = DatabasePDG->GetParticle(pdg1), *kaon = DatabasePDG->GetParticle(pdg2);

  WriterAscii hepmc_output("out.hepmc");
  int events_parsed = 0;
  ...
  for (events_parsed = 0; events_parsed < n_events; events_parsed++) {
    ...
    for(unsigned iq=0; iq</*2*/2; iq++) {
      Double_t eta   = rdmn_gen->Uniform(et1, et2);
      Double_t th    = 2*std::atan(exp(-eta));
      Double_t p     = rdmn_gen->Uniform(pp1, pp2);
