...
void hepmc_writer_two_particles_user(float pp, float et, float pdg1, float pdg2, int n_events)
{
  ...
  for (events_parsed = 0; events_parsed < n_events; events_parsed++) {
    ...
    for(unsigned iq=0; iq</*2*/2; iq++) {
      Double_t eta   = rdmn_gen->Uniform(et, et+0.01);
      Double_t th    = 2*std::atan(exp(-eta));
      Double_t p     = rdmn_gen->Uniform(pp, pp+0.01);
