										 ...
										 const std::vector<MisalignedLocation2D> &xycoord)
{
...

  {
    ...
    // A single entry; this assumes of course that all the windows are at the same Z, and parallel to each other;
    auto surface = new FlatSurface(sign*(1/mm)*TVector3(0,0,fvzOffset + wzOffset + _HRPPD_WINDOW_THICKNESS_/2), nx, ny);
  ...
  //---------------------------------
  // 1-side pyramid mirror
  //--------------------------------- 
  CherenkovMirror *pyramid = 0;
  double pZ=50*mm, pY=40*mm, pX=18*mm, pLTX=9*mm;
  
  if (dbox->m_UsePyramids) {
    std::cout<<"*** build 1-side pyramid"<<std::endl;

    // How to use G4Trap -- https://geant4-forum.web.cern.ch/t/g4trap-origin-position/5642/7 
    auto pyra_trap=new G4Trap("pyra_trap",pZ, pY, pX, pLTX);
  ...
  m_Geometry->AddPhotonDetector(cdet, pd->GetLogicalVolume(), pd);

   {
    ...
    //Mirror placement
    double rotX[4]={ 90,   0,  90,  90};
    double rotY[4]={180, -90,  90,   0};
    double rotZ[4]={  0,  90,   0,   0};

    double shift=_HRPPD_ACTIVE_AREA_SIZE_/2.+(3.*pX-pLTX)/4.;
    double shiftX[4]={shift,    0,      0, -shift};
    double shiftY[4]={    0,shift, -shift,      0};
    ...
    for(auto xyptr: xycoord ) {
      ...
      // Placing the pyramid mirror
      if (dbox->m_UsePyramids && xyptr.m_Pyramid) {
	G4RotationMatrix* Rotation = new G4RotationMatrix();
        Rotation->rotateX(rotX[iMirror]*deg);
        Rotation->rotateY(rotY[iMirror]*deg);
        Rotation->rotateZ(rotZ[iMirror]*deg);
        auto pyra_phys =new G4PVPlacement(Rotation, G4ThreeVector(xyptr.m_X+shiftX[iMirror], xyptr.m_Y+shiftY[iMirror], dbox->m_gas_volume_length/2 - dbox->m_PyramidMirrorHeight/2),
					  pyramid->GetLogicalVolume(), pyramid->GetSolid()->GetName(),
					  dbox->m_gas_volume_phys->GetLogicalVolume(), false, counter);
	new G4LogicalBorderSurface(pyramid->GetSolid()->GetName(), dbox->m_gas_volume_phys, pyra_phys, pyramid->GetMirrorSurface());
	iMirror++;
...

      {
		...
		auto qsurface = new FlatSurface((1/mm)*center, nx, ny);
		auto boundary = new OpticalBoundary(m_Geometry->FindRadiator(dbox->m_gas_volume_phys->GetLogicalVolume()), qsurface, false);
                pboundaries[iq] = boundary;
                cdet->StoreOpticalBoundary(boundary);
              }
              break;
	...
	for(unsigned iq=0; iq<4; iq++) {
	  ...
	  for(unsigned ip=0; ip<5; ip++) {
	    ...
	    switch (iq) {
            case 0: // Direct hit;
	      break;
	    case 1: // Reflection on either inner or outer mirrors;  
            case 2: // Reflection on either inner or outer mirrors;
	      irt->AddOpticalBoundary(dbox->m_mboundaries[iq-1]);
	      break;
	    case 3: // Reflection on outer, then on inner mirror; happens at large angles; if the pyramids are
	            // too high, these photons will undergo more reflections, and cannot be saved;
	      irt->AddOpticalBoundary(dbox->m_mboundaries[1]);
              irt->AddOpticalBoundary(dbox->m_mboundaries[0]);
              break;
